...
class multi_skf(imdb):
    def __init__(self, image_set,  devkit_path=None):
        imdb.__init__(self, 'multi_skf_' +  image_set)
        #self._year = year
    ...
    def image_path_from_index(self, index):
        ...
        image_ext = '.png' if 'KITTI' in index else '.jpg'
        image_path = os.path.join(cfg.DATA_DIR, index + image_ext)
    ...
    def _get_default_path(self):
        ...
        """
        return os.path.join(cfg.DATA_DIR, 'multi_skf')
    ...
    def _load_pascal_annotation(self, index):
        ...
        filename = os.path.join(cfg.DATA_DIR, index.replace('JPEGImages','Annotations') + '.xml')
        #print(filename)
        tree = ET.parse(filename)
        objs = tree.findall('object')
        sizes = tree.findall('size')
        width = float(sizes[0].find('width').text)
        height = float(sizes[0].find('height').text)
    ...
    def _get_voc_results_file_template(self,custermized_name):
        ...
        filedir = os.path.join(cfg.ROOT_DIR, 'results', custermized_name, self.name, 'det_cls_txt')
